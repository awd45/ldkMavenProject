  ...
  def main(args: Array[String]): Unit = {
    ...
    def smoothing(beeguArray: Array[Double]): Array[Double] = {
      ...
      for (index <- 0 until subsmcount) {
        ...
        var smfirst = (smDataFirst.sum / smDataFirst.size).round
    ...
    var middleResult2 = groupRddMapExp.toDF("REGIONID","PRODUCT","WEEK","VOLUME","MOVINGVALUE","RATIO","STDEVMA","MAXVALUE","MINVALUE","REFINEDQTY","SMOOTHINGNIE")
    ...
    var finalResultDf = spark.sql("select REGIONID,PRODUCT,WEEK, VOLUME, MOVINGVALUE, RATIO, STDEVMA, MAXVALUE, MINVALUE" +
      ",REFINEDQTY, SMOOTHINGNIE from middleTable groupby REGIONID,PRODUCT,WEEK")
