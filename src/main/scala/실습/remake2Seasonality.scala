...
object remake2Seasonality {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var middleResult5 = groupRddMapExp5.toDF("REGIONID", "PRODUCT", "WEEK", "VOLUME", "MOVINGVALUE", "RATIO", "STDEVMA", "MAXVALUE", "MINVALUE","REFINEDQTY", "SMOOTHINGNIE")
    ...
    //    val middleResult5 = spark.createDataFrame(groupRddMapExp5,
    //      StructType(Seq(StructField("REGIONID", StringType),
    //        StructField("PRODUCT", StringType),
    //        StructField("WEEK", StringType),
    //        StructField("VOLME", StringType),
    //        StructField("MOVINGVALUE", StringType),
    //        StructField("RATIO", StringType),
    //        StructField("STDEVMA", StringType),
    //        StructField("JUNGJAEGOOD", StringType),
    //        StructField("SMOOTHINGNIE", StringType))))

    middleResult5.createOrReplaceTempView("middleTable")
    // 데이터프레임을 디버깅하고 .show()로 본다.

    var finalResultDf = spark.sql("select REGIONID, PRODUCT, WEEK, VOLUME, MOVINGVALUE, MAXVALUE, MINVALUE "+
      " AVG(RATIO) AS AVG_RATIO, STDEVMA, REFINEDQTY, SMOOTHINGNIE from middleTable group by REGIONID, PRODUCT, WEEK, VOLUME, MOVINGVALUE, STDEVMA, REFINEDQTY, SMOOTHINGNIE ")
    ...
    // 저장하기
    var outUrl = "jdbc:oracle:thin:@192.168.110.112:1521/orcl"
    var outUser = "kopo"
    var outcPw = "kopo"
